...
def DE_task(pop, param, func_idx,
    ...
    opt_cnt = differential_evolution(pop=pop,
                                     param=param,
                                     func_id=func_idx)
    ...
    print("%d-%d" % (func_idx, i))
    return opt_cnt, mean_PR_table, mean_NSR_table

def crowding():
    run = 50
    accuracy_filename = "DE_accuracy_PR_SR.csv"
    opt_filename = "DE_generation_optima_count.csv"
    ...
    opt_record = []
    crossover_operator = [heuristic_crossover, one_bit_crossover, arithmetic_crossover]
    for func_idx in function_idx:
        ...
        p = Pool()
        for i in range(run):
            result.append(p.apply_async(DE_task, args=(pop, param, func_idx,
                                                      mean_PR_table, mean_NSR_table, benchmark,
                                                      accuracy_level, func, i)))
        p.close()
        p.join()
