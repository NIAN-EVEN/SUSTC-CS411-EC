...
def optimise(pop, benchmark, budget, config, selection, recombination, mutation, filename):
    ...
    while evaluate_num < budget:
        ...
        # mutation_offsprings = []
        # for i in range(int(offspring_size*mutation_rate)):
        #     mutation_offsprings.extend(random.choice(offsprings))
...
def truncated_selection(pop, n, _):
    pop.sort(key=lambda x: x.fitness)
    return pop[:n]
...
def dbg():
    print(os.getcwd())
    iterNum = 10
    for benchmark, budget in zip(benchmark_lst[1], moreevaluations[1]):
...
recombination_set = [arithmetic_recombination, discrete_recombination, global_discrete_recombination, one_bit_crossover]

mutation_set = [cauchy_mutation, gaussian_mutation, uniform_mutation, one_bit_mutation]

selection_set = [rank_based_selection, round_robin_selection, tournament_selection, truncated_selection]
