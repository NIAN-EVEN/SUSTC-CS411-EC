...
def dbg():
    ...
    for benchmark, budget in zip(benchmark_lst, evaluations):
        ...
        for k, pop in enumerate(pop_list):
            record, pop0 = optimise(pop, benchmark, budget, config, None,
                                    None, None, filename=None)
            print('Function: %s RUN %d: Approximate optimal value=%.16f' % (benchmark[0].__name__, k, pop0.fitness))
            # print('Function: %s RUN %d: Approximate optimum=%s' % (benchmark[0].__name__, k, str(pop0.vec)))
